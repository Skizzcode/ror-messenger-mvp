...
export default function FanDashboard(){
  ...
  useEffect(() => {
    setMounted(true);
    t('page_view', { scope: 'fan_dashboard' });
  }, []);

  useEffect(() => {
    t('fan_wallet_state', { scope: 'fan_dashboard', props: { connected: !!pubkey } });
  }, [pubkey]);
          ...
          {pubkey && (
            <>
              ...
              <Tabs
                ...
                onSwitch={(key)=> t('fan_tab_switch', { scope:'fan_dashboard', props:{ tab: key } })}
                renderItem={(tItem:any)=>(
                  <div key={tItem.id} className="p-3 rounded-xl border border-white/10 flex items-center justify-between">
                    <div>
                      <div className="font-semibold">{tItem.id.slice(0,8)}…</div>
                      <div className="text-xs text-muted">
                        {tItem.status.toUpperCase()} · {tItem.messagesCount} msgs
                    ...
                    <Link
                      href={`/c/${tItem.id}`}
                      className="btn"
                      onClick={() => t('fan_open_chat_click', { scope:'fan_dashboard', props:{ threadId: tItem.id } })}
                    >
                      Open chat
                    </Link>
                  </div>
...
}:{ tabs: {key:string; label:string; items:any[]}[]; renderItem:(x:any)=>ReactNode; onSwitch:(key:string)=>void }){
